...
def get_data(params, header, _start):
    ...
    try:
        ...
        for i in range(_start, 20000, 100):
            ...
            for j in range(100):
                ...
                with conn.cursor() as cursor:
                    cursor.execute(
                        f"update dota set buy_price = {buy_price}, amount_items_on_steam = {sell_listings} where hash_name='{hash_name}';"
                    )
            print(_start)
            time.sleep(30)
        ...
        print(ex)
        time.sleep(40)
        get_data('&count=100&search_descriptions=0&sort_column=popular&sort_dir=desc&appid=570&norender=1', headers,
                 start)
    finally:
        start = 1
        get_data('&count=100&search_descriptions=0&sort_column=popular&sort_dir=desc&appid=570&norender=1', headers,
...
def main():
    get_data('&count=100&search_descriptions=0&sort_column=popular&sort_dir=desc&appid=570&norender=1', headers, 1)
